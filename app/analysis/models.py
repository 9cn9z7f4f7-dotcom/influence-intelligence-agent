...
class PlatformCoverage(BaseModel):
    ...
    # Additive funnel counters. ``items_collected`` keeps its original meaning:
    # raw discovery candidates returned by the source. The fields below expose
    # what happened after deterministic classification without changing any
    # classifier thresholds or calculations.
    items_checked: int = 0
...
class AnalysisFinding(BaseModel):
    """Presentation-ready real finding for the data-dense "Что нашли" view.

    This is an additive projection of already collected ``Integration`` /
    ``PotentialCreatorSignal`` records. It deliberately does not introduce new
    scoring or classification logic; every row keeps a real source URL and the
    evidence IDs generated by the existing pipeline.
    """

    finding_id: str
    entity_type: Literal["creator", "publisher", "page"]
...
class AnalysisResult(BaseModel):
    ...
    # Persisted with the analysis so evidence IDs remain resolvable after a
    # process restart. The public GET /api/analysis/{id} response excludes this
    # potentially large map; callers use the analysis-scoped evidence endpoint.
